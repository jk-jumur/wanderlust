...
const BookingCard = ({ destination }) => {
  ...
  const handleBooking = async () => {
    ...
    if (!user) {
      toast.error("Please log in to book this resort!");
      return;
    }
   ...
   console.log(tokenData)
    try {
      ...
      // 2. Success/Error Toast with Dynamic Destination Name
      if (res.ok && data.insertedId) {
        toast.success(`Successfully booked ${destinationName || "your resort"}! 🎉`);
      } else {
        toast.error(`Failed to book ${destinationName || "resort"}. Please try again.`);
      }
    } catch (error) {
